<template>
    <div class="pizza-block">
        <img :src="image" alt="Pizza" class="pizza-block__image" />
        <h4 class="pizza-block__title">{{ title }}</h4>
        <div class="pizza-block__selector">
            <ul>
                <li v-for="thickness in ['тонкое', 'традиционное']" :class="{ 'active': thickness === selectedThickness }"
                    @click="selectedThickness = thickness" v-text="thickness" />
            </ul>
            <ul>
                <li v-for="diameter in [26, 30, 40]" :class="{ 'active': diameter === selectedDiameter }"
